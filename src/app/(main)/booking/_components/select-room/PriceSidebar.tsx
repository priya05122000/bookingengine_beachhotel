...
const CARD = "bg-primary/4 rounded-xs shadow-[1px_4px_4px_0px_#00000040] p-4";
    ...
    <div key={item.id} className="border-b last:border-b-0 pb-2">
      <div className="flex items-stretch justify-between">
        <div className="flex-1 space-y-1">
          ...
          <div
            className={`${FONT} text-black mt-1 bg-silver/30 inline-block p-1`}
          >
            {item.details}
          </div>
          <div className={`${FONT} text-dark-gray mt-1`}>
            {item.rooms} Room{item.rooms > 1 ? "s" : ""} · {item.adults} Adult
            {item.adults > 1 ? "s" : ""}
            {item.children > 0
...
export default function PriceSidebar({
  ...
  return (
    <div className="space-y-3 xl:bg-white xl:p-3 shadow-[1px_4px_4px_0px_#00000040] rounded">
      <div className={CARD}>
        {selected.length > 0 ? (
          ...
          <div
            className={`${FONT} text-dark-gray flex space-y-1 items-center justify-between py-3`}
          >
        ...
        <div
          className={`${FONT} text-dark-gray border-t py-3 flex items-center justify-between`}
        >
          <div>Hotel Taxes</div>
        ...
        <div
          className={`${typography.textLg} font-arizona-light uppercase border-t-3  border-black pt-2 flex items-center justify-between `}
        >
        ...
        {selected.length > 0 && (
          <>
            <div className="mt-4">
              <label className={`${FONT} text-dark-gray mb-1 block`}>
                Promo Code
              </label>
              <input
                value={promo}
      ...
      {!hideLoginCard && (
        <div className={CARD}>
          <div className={`${FONT} mb-3 block font-arizona-sans-regular`}>
            <Link
              href="/signin"
              className="mr-1 text-xs lg:text-sm underline underline-offset-4 text-primary hover:text-primary transition-colors cursor-pointer"
            >
              LOGIN
            </Link>
            or
            <Link
              href="/signup"
              className="ml-1 text-xs lg:text-sm underline underline-offset-4 text-primary hover:text-primary transition-colors cursor-pointer"
            >
              SIGN UP
            </Link>
          </div>
          ...
          <ul
            className={`text-xs lg:text-sm tracking-wide text-dark-gray space-y-2`}
          >
